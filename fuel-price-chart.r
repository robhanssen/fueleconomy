...
predict_fuel_price <-
    ...
    lm(price ~ date, data = .) %>%
    broom::augment(newdata = tibble(date = seq(cutoff_date - 39, invasion + 40, 1))) %>%
    rename(price = ".fitted")
...
fuel %>%
    ...
    geom_line(data = predict_fuel_price, lty = 2, color = "gray50", alpha = .7) + 
    geom_vline(xintercept = as.Date("2021-01-21"), lty = 3) +
    geom_vline(xintercept = as.Date("2022-02-24"), lty = 3) +
    annotate("label", x = as.Date("2021-01-21"), y = .20, label = "Inauguration\nBiden") +
    annotate("label", x = as.Date("2022-02-24"), y = .20, label = "Invasion of\nUkraine") +
    theme_light() 
